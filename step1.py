...
import numpy as np
import os
os.chdir(r"C:\Users\andre\Desktop\scripts\PT")

# Leitura do CSV com parse das datas
df = pd.read_csv (r'CasosPortugal_15maio_23jun.csv', sep=';', thousands=',', parse_dates=['DATA'], dayfirst=True) #, header=None, names=headers, dtype=dtypes
#print (df)

# Selecao da data base para a predicao
...
# Ciclo para estimar todas os dias existentes na BD com 14 dias de historico 
for i, row in df.iterrows():
    start_date = df.at[i,'DATA']
    #print("A data para estimativa a 14 dias e: " + str(start_date))
    
    date_1 = pd.to_datetime(start_date) + pd.DateOffset(days=-29)
    #print("A data 1 e: " + str(date_1))
    end_date1 = pd.to_datetime(date_1) + pd.DateOffset(days=+15)
    #print("O fim do periodo 1 e: " + str(end_date1))
    ...
    if not df_X.empty:
        ...
        #print("A referencia temporal de NC e: " + str(date_2))
        ...
        #print("A referencia temporal de NC e: " + str(end_date3))
        ...
        if not df_Y.empty and len(df_X)==len(df_Y):
            ...
            Y = df_Y['NC'] # Numero de casos diarios
            ...
            #print (df_predicted)
            # Valores das variaveis para os 14 dias a estimar
            estimado = 0
            observado = 0
            erro_obs = 0
            ...
            print("A referencia temporal do estimado e: " + str(datareferencia))
            #estimado = int(regr.predict([[default_RR , default_GPH, default_P, default_T, default_WP, default_RS]]))            
            print ('Estimado: ' + str(estimado))
            
            #Ciclo para estimar o numero de casos para o dia n de um conjunto de 14
